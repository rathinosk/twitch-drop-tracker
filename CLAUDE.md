...
- `extension-name-config.yaml` lists the files included when packaging a distributable zip (manifest, background, content scripts, i18n, `fullpage/`, `icons/`, `locales/`, `popup/`, `shared/`).
...
- **`shared/view-core.js`** — ES module holding all data loading, filtering, and rendering logic for the two UIs (`renderCampaigns`, `renderCampaignCard`, `renderDropItem`, `renderMyProgress`, `gameNameToSlug`, the game filter, etc.). `popup/popup.js` and `fullpage/fullpage.js` are both thin `type="module"` wrappers: each calls `configure({ campaignsContainerId, progressContainerId, filterAnimated, lastUpdated })` to declare its layout's element IDs and per-layout quirks, then wires only what's genuinely page-specific (popup: settings panel, RAM-mode toggle, diagnostic-log copy, clear-cache; fullpage: none beyond the shared calls — its search box and stats bar are handled inside the shared module and no-op automatically when the corresponding elements don't exist, e.g. in the popup). **Visual sizing (icon dimensions, spacing, font sizes) is CSS-only** — `popup.css`/`fullpage.css` each define their own values for shared classes like `.expand-icon`, `.section-chevron`, `.empty-state-hint`; the shared module never hardcodes pixel sizes, so a layout tweak only ever touches CSS/HTML, not `view-core.js`.
